...
export const dynamic = 'force-dynamic'; // ✅ Force SSR (no pre-rendering)
...
const Dashboard = () => {
  ...
  // ✅ Move fetchPasswords above useEffect so it's defined before use
  ...
  const handleDelete = async () => {
    if (!session?.user || !deleteConfirm.id) {
      ...
      <div className="bg-[#1a1a1d] rounded-xl p-6 shadow-lg border border-gray-700 hover:border-[#6C5CE7] transition-all duration-300 transform hover:-translate-y-1">
        <div className="flex justify-between items-start mb-4">
          ...
          <div className="flex space-x-2">
            <button onClick={() => handleEdit(password)} className="text-gray-400 hover:text-blue-400"><FaEdit /></button>
            <button onClick={() => confirmDelete(password._id)} className="text-gray-400 hover:text-red-400"><FaTrash /></button>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between p-1 rounded-lg">
            ...
            <div className="flex space-x-2">
              <button onClick={() => setShowPassword(!showPassword)} className="p-2 rounded-full text-gray-400 hover:bg-gray-700">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        {deleteError && (
          <div className="mb-6 p-4 bg-red-900/30 border border-red-700 rounded-lg text-red-200 flex items-center">
            ...
            <button onClick={() => setDeleteError(null)} className="ml-auto text-red-400 hover:text-red-200">×</button>
          </div>
        )}
        ...
        {deleteConfirm.show && (
          <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50">
            <div className="bg-[#1a1a1d] p-6 rounded-xl text-center w-auto shadow-lg border border-gray-700">
              ...
              <p className="text-gray-400 mb-6">Are you sure you want to delete this password? This action cannot be undone.</p>
              <div className="flex justify-center gap-4">
                <button onClick={() => setDeleteConfirm({ show: false, id: null })} className="px-4 py-2 bg-gray-700 rounded-lg hover:bg-gray-600 transition">Cancel</button>
                <button onClick={handleDelete} className="px-4 py-2 bg-red-600 rounded-lg hover:bg-red-500 transition">OK</button>
              </div>
            </div>
          </div>
        )}
